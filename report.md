...
**(a) Per-partition Paxos contention.** Cassandra scopes Paxos state per partition
key, not per row. Every seat of a departure shares one partition, so concurrent
bookings on *different* seats of the same departure still serialize through that
partition's single Paxos lane. This — not storage hotness — is the real reason a
popular departure shows modest booking throughput and frequent CAS retries under
ST3. For this project that's desirable: it's exactly what makes ST3's contention
real. The takeaway for scale: LWT booking throughput is bounded per departure; you'd
shard hot departures across partitions to lift the ceiling, at the cost of the
single-partition seat-map read.

**(b) The `applied=False` return shape.** It's tempting to assume a failed
conditional `UPDATE` returns the current row so you can see who holds the seat. It
does not — it returns only the columns named in the `IF` clause. Returning the whole
row is `IF [NOT] EXISTS` behavior, which we deliberately avoid. The fix is the
explicit read-back, which doubles as the idempotency check.

**(c) The ambiguous LWT write-timeout.** A `WriteTimeout` on an LWT is the one real
correctness trap: the Paxos round may or may not have committed, so a blind retry is
unsafe (it could book a second seat or report a false conflict). We resolve it with
the same read-back primitive: read the seat at `LOCAL_SERIAL`; if it carries this
request's `idempotency_key`, the round committed → success; if it's booked by someone
else → 409; if it's still free, the round didn't commit → safe to retry (bounded).
This is the only place where retry/backoff and the read-back interact.

**(d) Retry discipline.** Bounded exponential backoff with full jitter applies only
to transient/availability errors (`Unavailable`, `OperationTimedOut`, `ReadTimeout`).
It is never applied to a 409 — that's a definitive answer ("seat taken"), and
retrying it on the same seat would spin forever. ST3 relies on this: a 409 means
"move to the next seat" and only transient errors trigger retry, which is why all
seats sell and none is abandoned.

**(e) LWT latency cost.** A booking runs one Paxos round (propose/accept) plus the
commit, so it costs materially more than a plain `LOCAL_QUORUM` read. To quantify the
"no big delays" requirement, measure the gap between LWT booking latency and plain
read latency (compare `POST /reservations` p50/p95 against `GET /reservations/...`
p50/p95 from the same run). That gap is the concrete price of strong consistency on
the booking path — the number to cite.

**(f) Node-failure behavior.** With RF=3 and `LOCAL_QUORUM` (2 of 3), killing one
Cassandra node leaves a quorum of replicas, so reads and writes continue and no data
is lost; the LWT's `LOCAL_SERIAL` quorum is likewise still reachable. Killing one app
instance is invisible to clients — Nginx routes around it, and the surviving apps
contact all three Cassandra nodes anyway. Killing a *second* Cassandra node would
break quorum, and the booking path would correctly start returning `503 Unavailable`
rather than risk an unsafe write: availability is sacrificed before safety.

**(g) Per-instance state behind the load balancer.** The demo test-runner first kept
...
the losers got 80 clean 409s, all seats sold, zero double-bookings, and *both*
